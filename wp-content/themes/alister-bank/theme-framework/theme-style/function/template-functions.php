...
function alister_bank_get_header_search_form($cmsmasters_option) {
	if (
		$cmsmasters_option['alister-bank' . '_header_search'] && 
		$cmsmasters_option['alister-bank' . '_header_styles'] != 'c_nav'
	) {
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__('Enter Keywords', 'alister-bank') . "\" value=\"\" />
...
function alister_bank_theme_error_styles() {
	...
	if (is_404()) {
		...
		if (
			$cmsmasters_option['alister-bank' . '_error_bg_img_enable'] && 
			$cmsmasters_option['alister-bank' . '_error_bg_image'] != ''
		) {
			$error_bg_image = explode('|', $cmsmasters_option['alister-bank' . '_error_bg_image']);
			...
			$out .= "
	.error .error_bg {
		background-image : " . (!empty($cmsmasters_option['alister-bank' . '_error_bg_image']) ? 'url(' . ((is_numeric($error_bg_image[0])) ? $error_bg_image_url[0] : $error_bg_image[1]) . ')' : 'none') . ";
		background-position : " . (!empty($cmsmasters_option['alister-bank' . '_error_bg_pos']) ? $cmsmasters_option['alister-bank' . '_error_bg_pos'] : 'top center') . ";
		background-repeat : " . (!empty($cmsmasters_option['alister-bank' . '_error_bg_rep']) ? $cmsmasters_option['alister-bank' . '_error_bg_rep'] : 'repeat') . ";
		background-attachment : " . (!empty($cmsmasters_option['alister-bank' . '_error_bg_att']) ? $cmsmasters_option['alister-bank' . '_error_bg_att'] : 'scroll') . ";
		background-size : " . (!empty($cmsmasters_option['alister-bank' . '_error_bg_size']) ? $cmsmasters_option['alister-bank' . '_error_bg_size'] : 'auto') . ";
	}
";
		}
		...
		$out .= "
	.error .error_bg {
		" . cmsmasters_color_css('background-color', $cmsmasters_option['alister-bank' . '_error_bg_color']) . "
	}
	
	.error .error_title, 
	.error .error_subtitle {
		" . cmsmasters_color_css('color', $cmsmasters_option['alister-bank' . '_error_color']) . "
	}
";
	}
...
function alister_bank_page_heading() {
	if (is_404() || is_home()) {
		...
		if (
			...
			$cmsmasters_header_overlaps = $cmsmasters_option['alister-bank' . '_header_overlaps'] ? 'true' : 'false';
			
			$cmsmasters_heading_alignment = $cmsmasters_option['alister-bank' . '_heading_alignment'];
			$cmsmasters_heading_scheme = $cmsmasters_option['alister-bank' . '_heading_scheme'];
			
			$cmsmasters_breadcrumbs = $cmsmasters_option['alister-bank' . '_breadcrumbs'] ? 'true' : 'false';
		}
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'alister-bank')), $wp_query->found_posts, get_search_query());
						} else {
							echo sprintf(esc_html__('0 search results for: %s', 'alister-bank'), get_search_query());
						}
					} elseif (is_archive()) {
						if (is_author()) {
							if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
								echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'alister-bank'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
							} else {
								echo sprintf(esc_html__('Author: %s', 'alister-bank'), '<span class="vcard">' . get_the_author() . '</span>');
							}
						} else {
...
function alister_bank_theme_heading_styles() {
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['alister-bank' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['alister-bank' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['alister-bank' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['alister-bank' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['alister-bank' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['alister-bank' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['alister-bank' . '_heading_bg_size'];
	}
...
function alister_bank_thumb_rollover($cmsmasters_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false, $link_target = false, $placeholder_icon = 'cmsmasters_theme_icon_image', $puzzle_image = '') {
	...
	if ($rollover) {
		...
		$out .= '<div class="cmsmasters_img_rollover">' . 
			'<a href="' . (($link_redirect == 'true' && $link_url != '') ? esc_url($link_url) : esc_url(get_permalink($cmsmasters_id))) . '"' . (($link_redirect == 'true' && $link_target == 'true') ? ' target="_blank"' : '') . ' title="' . esc_attr($cmsmasters_title) . '" class="cmsmasters_open_post_link button">' . esc_html__('Read more', 'alister-bank') . '</a>' . 
		'</div>';
	} else {
		$out .= '<div class="cmsmasters_img_rollover">' . 
...
function alister_bank_get_page_classes($cmsmasters_option, $classes = false) {
	...
	echo 'cmsmasters_' . $cmsmasters_option['alister-bank' . '_theme_layout'] . ' ';
	...
	if ($cmsmasters_option['alister-bank' . '_fixed_header']) {
		echo 'fixed_header ';
	}
	
	
	if ($cmsmasters_option['alister-bank' . '_header_top_line']) {
		echo 'enable_header_top ';
	}
	
	
	if ($cmsmasters_option['alister-bank' . '_header_styles'] != 'default') {
		echo 'enable_header_bottom ';
	}
	
	
	if ($cmsmasters_option['alister-bank' . '_header_styles'] == 'r_nav') {
		echo 'enable_header_right ';
	}
	
	
	if ($cmsmasters_option['alister-bank' . '_header_styles'] == 'c_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['alister-bank' . '_header_overlaps'];
	}
...
function alister_bank_theme_header_styles() {
	...
	$header_top_height = (($cmsmasters_option['alister-bank' . '_header_top_height'] !== '') ? $cmsmasters_option['alister-bank' . '_header_top_height'] : $defaults[$tab]['alister-bank' . '_header_top_height']);
	...
	$header_bot_height = (($cmsmasters_option['alister-bank' . '_header_bot_height'] !== '') ? $cmsmasters_option['alister-bank' . '_header_bot_height'] : $defaults[$tab]['alister-bank' . '_header_bot_height']);
...
";
	...
	if ($cmsmasters_option['alister-bank' . '_custom_css'] != '') {
		$out .= stripslashes($cmsmasters_option['alister-bank' . '_custom_css']);
	}
