...
function alister_bank_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['alister-bank' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date">' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
				'</abbr>' . 
...
function alister_bank_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'alister-bank') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
		...
		if ($cmsmasters_option['alister-bank' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'alister-bank') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
					'<span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
				'</a>' . 
...
function alister_bank_get_post_tags($show = true) {
	if (get_the_tags()) {
		...
		if ($cmsmasters_option['alister-bank' . '_blog_post_tag']) {
...
function alister_bank_get_post_likes($template_type = 'page', $show = true) {
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['alister-bank' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function alister_bank_get_post_comments($template_type = 'page', $show = true) {
	...
	if (comments_open()) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['alister-bank' . '_blog_post_comment']) {
				$out = alister_bank_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function alister_bank_post_more($cmsmasters_id, $show = true) {
	...
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Read more...', 'alister-bank');
	}
